...
def run_self_test() -> dict:
    ...
    print(f"global grid: side={config.global_grid_side_m} m, "
        f"points={len(grid.points)}, triangles={grid.triangle_count}, "
        f"route={grid.route_distance_m:.1f} m")
    ...
    for _ in range(1000):
        ...
        assert any(
            any(
                math.dist(source, vertex) <= config.reception_radius_min_m + 1e-6
