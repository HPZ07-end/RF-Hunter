...
在线运行：
    python question_4_global_scheduler.py --robot-id 你的参赛队号

离线自检：
    python question_4_global_scheduler.py --self-test

第四版重点：
* 900 米等边三角网格负责“不会仅因朝向而漏检”。
* 每到达一个全局网格点，先检测未知频道，再利用当前位置为已发现频道补充测向。
* 补测采用接收概率、定位区域缩减量与短期路线前瞻筛选，并限制每点及每频道次数。
* 补测使定位区域达到清除条件后只登记 READY；除 near 外不在搜索途中离开网格清除。
...
class Q4Config:
    ...
    # 问题四保证参数
    global_grid_side_m: float = 900.0
    ...
    def __post_init__(self):
        ...
        if self.global_grid_side_m > self.reception_radius_min_m:
            raise ValueError("粗三角网格边长不能超过保证接收半径1000米")
...
def run_self_test() -> dict:
    ...
    grid = build_global_grid(config)
    assert grid.triangle_count == 54, grid.triangle_count
    assert len(grid.points) == 37, len(grid.points)
    assert abs(grid.route_distance_m - 32400.0) <= 1e-6, grid.route_distance_m
    ...
    for _ in range(1000):
        ...
        containing = [triangle for triangle in grid.triangles if point_in_triangle(source, triangle)]
        assert containing, source
        assert any(
            all(math.dist(source, vertex) <= config.global_grid_side_m + 1e-6 for vertex in triangle)
            and any(
    ...
    return {
        ...
        "global_grid_triangles": grid.triangle_count,
        "global_route_distance_m": grid.route_distance_m,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="关闭全局搜索途中的已发现频道补测，用于基线对照",
    )
    parser.add_argument("--opportunistic-max-per-node", type=int, default=2)
    parser.add_argument("--opportunistic-max-per-channel", type=int, default=3)
    parser.add_argument("--opportunistic-lookahead-nodes", type=int, default=3)
    parser.add_argument(
    ...
    print(f"问题四第四版全局价值/耗时调度策略启动，输出目录：{output_dir}", flush=True)
    summary = runner.run()
