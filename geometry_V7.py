...
def _classify(rows, rational, cfg, diag):
    ...
    try:
        ...
        diag["warnings"].append(
            f"线性规划调用异常({type(exc).__name__}: {exc})，回退到投影边界判定")
        return exact_state
    diag["lp_statuses"].append({"objective": (0, 0), "status": int(result.status),
                                "message": getattr(result, "message", "")})
    if result.status not in (0, 2, 3):
        diag["warnings"].append("线性规划求解失败，回退到投影边界判定")
        return exact_state
    if result.status == 2:
        if exact_state != "EMPTY":
            diag["warnings"].append("LP 报不可行，但投影边界判定非空，回退到投影边界")
        return exact_state
    if result.status == 3:
        if exact_state != "UNBOUNDED":
            diag["warnings"].append("LP 报无界，但投影边界判定有界，回退到投影边界")
        return exact_state
    if exact_state == "EMPTY":
        diag["warnings"].append("LP 找到可行点，但投影边界判定为空，回退到投影边界")
        return exact_state
    if getattr(result, "x", None) is None:
        diag["warnings"].append("LP 未返回可行点，回退到投影边界判定")
        return exact_state
    ...
    if residual > _tol(cfg, "feasibility", scale):
        diag["warnings"].append("线性规划返回点未通过约束残差检查，回退到投影边界")
        return exact_state
    return exact_state
...
def _vertices(rows, rational, cfg, diag):
    ...
    * 总复杂度典型 O(m·h)，最坏 O(m²)，显著低于 O(m³)。
    ...
    xb = _projection_bounds(rows, rational, swap=False, diag=diag)
    yb = _projection_bounds(rows, rational, swap=True, diag=diag)
    ...
    if xlo is None or xhi is None or ylo is None or yhi is None:
        raise ArithmeticError("增量裁剪前置 BOUNDED 判定失效")
    # 快速投影可能返回浮点边界；从初始顶点起统一使用 Fraction，
    # 避免混合运算产生舍入误差后被严格约束检查误判为越界。
    M_x = max(abs(Fraction(xlo)), abs(Fraction(xhi))) + 1
    M_y = max(abs(Fraction(ylo)), abs(Fraction(yhi))) + 1
