...
# ==================== 五（前置）精确旋转卡壳辅助函数 ====================
def _cross3(o, a, b):
    """(a-o) 与 (b-o) 的精确叉积。Fraction 输入时保持精确。"""
    return (a[0] - o[0]) * (b[1] - o[1]) - (a[1] - o[1]) * (b[0] - o[0])


def _sq_dist(a, b):
    """精确距离平方。Fraction 输入时保持精确。"""
    return (a[0] - b[0]) ** 2 + (a[1] - b[1]) ** 2


def _ccw_order(exact):
    """返回按质心极角逆时针排序的索引列表。

    仅用于凸多边形顶点；调用前需确保 exact 非退化（非共线）。
    """
    n = len(exact)
    cx = sum(p[0] for p in exact) / n
    cy = sum(p[1] for p in exact) / n
    cxf, cyf = float(cx), float(cy)
    return sorted(
        range(n),
        key=lambda k: math.atan2(
            float(exact[k][1]) - cyf,
            float(exact[k][0]) - cxf,
        ),
    )
...
def _diameter_rotating_calipers(points):
    """精确旋转卡壳，对 CCW 顺序的凸多边形求直径。

    输入: Fraction 坐标点列表, 长度 >= 3, 按逆时针顺序, 非退化。
    返回: (i, j, squared_distance) 三元组。
    距离平方与叉积均以有理数精确比较, 避免浮点并列最远点误判。
    """
    n = len(points)
    if n == 1:
        return 0, 0, Fraction(0)
    if n == 2:
        return 0, 1, _sq_dist(points[0], points[1])
    ...
    for i in range(n):
        ni = (i + 1) % n
        # 推进对踵点 j, 直到三角形 (i, ni, j) 面积不再增大
        while True:
            nj = (j + 1) % n
            cur = abs(_cross3(points[i], points[ni], points[j]))
            nxt = abs(_cross3(points[i], points[ni], points[nj]))
            if nxt > cur:
                j = nj
            else:
                break
        # 用当前对踵点 j 及其下一个顶点同时更新候选
        for k in (j, (j + 1) % n):
            d = _sq_dist(points[i], points[k])
            if d > best_sq:
                best_sq = d
                best_pair = (i, k)
            d = _sq_dist(points[ni], points[k])
            if d > best_sq:
                best_sq = d
                best_pair = (ni, k)

    return best_pair[0], best_pair[1], best_sq
...
def _measure(exact, vertices, rows, cfg, diag):
    ...
    # ---- 先用最远对判断 SEGMENT / POLYGON（与原逻辑一致） ----
    def squared(pair):
        i, j = pair
        return sum((a - b) ** 2 for a, b in zip(exact[i], exact[j]))

    i_brute, j_brute = max(combinations(range(len(exact)), 2), key=squared)
    A_brute, B_brute = exact[i_brute], exact[j_brute]
    cross_check = [
        (B_brute[0] - A_brute[0]) * (v[1] - A_brute[1])
        - (B_brute[1] - A_brute[1]) * (v[0] - A_brute[0])
        for v in exact
    ]
    is_segment = all(value == 0 for value in cross_check)

    # ---- 计算直径：SEGMENT 保留原枚举，POLYGON 走精确旋转卡壳 ----
    if is_segment:
        i, j = i_brute, j_brute
        distance_squared = squared((i, j))
    else:
        # 先按 CCW 排序；exact 与 vertices 必须同步重排
        order = _ccw_order(exact)
        exact = [exact[k] for k in order]
        vertices = [vertices[k] for k in order]
        i, j, distance_squared = _diameter_rotating_calipers(exact)
    ...
    # 同时保留精确系数模型下的覆盖结论
    exact_covers = all(
        sum((a - b) ** 2 for a, b in zip(v, midpoint)) <= distance_squared / 4
        for v in exact
    )
    scale = max(
        1.0, diameter, rmax,
        *(abs(v) for point in vertices for v in point),
    ...
    if not exact_covers and excess <= cover_tol:
        diag["warnings"].append(
            "精确系数模型略超出圆盘，但在覆盖容差内；布尔结果为近似接受"
        )

    status = "SEGMENT" if is_segment else "POLYGON"
    if is_segment:
        max_height = 0.0
    else:
        # 用排序后多边形的 A, B 重新算高度诊断量
        cross_ab = [
            (B[0] - A[0]) * (v[1] - A[1]) - (B[1] - A[1]) * (v[0] - A[0])
            for v in exact
        ]
        max_height = max(
            float(abs(value) / distance_squared) * diameter for value in cross_ab
        )
    diag["near_degenerate"] = (
        status == "POLYGON"
        and max_height <= _tol(cfg, "degeneracy", scale)
    )
    if diag["near_degenerate"]:
        diag["warnings"].append("区域近乎退化但仍有面积，保留POLYGON与所有顶点")

    # POLYGON 情况已在旋转卡壳前按 CCW 排序，此处不再重复排序
    if cfg.sort_vertices_ccw and status == "POLYGON":
        pass

    residual = max(
        math.fsum((a * x, b * y, -c)) for x, y in vertices for a, b, c in rows
    )
    ...
    if cover_tol > 1e-3:
        diag["warnings"].append(
            "坐标尺度导致覆盖容差超过1毫米，请按任务精度复核"
        )

    return GeometryResult(
        status, vertices, diameter,
