"""问题三 V2.3 主程序：七点覆盖、搜索定位交织、失败重定位与动态开放TSP。

同目录依赖（原文件内容无需修改，仅统一文件名）：
    client.py, geometry_V7.py, geometry_two_point_fast.py, planner.py
运行（模拟器已登录、已启动问题三测试且接口就绪）：
    python strategy_V2_3.py --robot-id 你的参赛队号
关闭插入对照：
    python strategy_V2_3.py --robot-id 你的参赛队号 --no-insert
关闭搜索-定位交织对照：
    python strategy_V2_3.py --robot-id 你的参赛队号 --no-interleave
参数对照：
    python strategy_V2_3.py --robot-id 你的参赛队号 --rho 1123 --interleave-detour 80
清除失败上限：
    python strategy_V2_3.py --robot-id 你的参赛队号 --max-clear-failures 3
...
class Q3Config:
    ...
    interleave_detour_threshold_s: float = 80.0
    max_interleaved_actions_per_leg: int = 2
    ...
    def __post_init__(self):
        ...
        if type(self.enable_interleaving) is not bool:
            raise ValueError("enable_interleaving 必须是布尔值")
        ...
        positive_integer_fields = (
            ...
            "max_interleaved_actions_per_leg",
            "source_samples",
            "candidate_count",
            "error_samples",
        )
...
def insertion_cost(position, clearance, next_point):
    extra = route_detour_distance(
        position,
        clearance,
        next_point,
    )
    return extra/5 + 5.  # clear不切频道


def route_detour_distance(position, action_point, next_point):
    """将一个行动点插入当前覆盖边所增加的移动距离。"""
    extra = (
        math.dist(position, action_point)
        + math.dist(action_point, next_point)
        - math.dist(position, next_point)
    )
    if extra < -1e-6:
...
def interleaved_action_cost(
    ...
    """覆盖边中插入行动的新增虚拟时间上界。"""
    return (
        route_detour_distance(
            position,
            action["point"],
            next_point,
        )
        / 5.0
        + action_operation_s(action, current_channel)
    )
...
class Q3Runner:
    ...
    def __init__(self, client, *, config=None, planner_config=None, geometry_config=None,
        ...
        self.counts = {
            ...
            "interleaved_measure": 0,
            "active_measure": 0,
            "recovery_measure": 0,
            "interleaved_action": 0,
            "interleaved_clear": 0,
            "interleave_candidate_rejected": 0,
    ...
    def interleaved_candidates(
        ...
        for channel, record in self.records.items():
            if record.status == "READY":
                ...
                cost_s = interleaved_action_cost(
                    self.client.position,
                    action,
                    next_coverage_point,
                    self.client.current_channel,
                )
                detour_s = max(0.0, cost_s - 5.0)
                threshold_s = self.cfg.insert_threshold_s
                if cost_s > threshold_s:
                    self.counts[
                        "interleave_candidate_rejected"
                    ] += 1
                    self.event(
                        "interleaved_candidate_rejected",
                        channel=channel,
                        action_type="CLEAR",
                        point=action["point"],
                        route_detour_s=detour_s,
                        incremental_cost_s=cost_s,
                ...
                candidates.append(
                    {
                        **action,
                        ...
                        "incremental_cost_s": cost_s,
                        "threshold_s": threshold_s,
                        "threshold_metric": "incremental_cost_s",
                        "diameter_reduction_m": None,
                        "utility": math.inf,
                    }
                )
                continue
            ...
            if (
                record.status != "FOUND"
                or channel in measured_channels
            ):
                continue
            ...
            if detour_s > self.cfg.interleave_detour_threshold_s:
                self.counts[
                    "interleave_candidate_rejected"
                ] += 1
                self.event(
                    ...
                    incremental_cost_s=cost_s,
                    threshold_s=(
                        self.cfg.interleave_detour_threshold_s
                    ),
                )
                continue
            ...
            reduction = max(
                0.0,
                record.diameter - proposal["score"],
            )
            candidates.append(
                {
                    **action,
                    ...
                    "threshold_metric": "route_detour_s",
                    "diameter_reduction_m": reduction,
                    "utility": reduction / max(cost_s, 1e-9),
                }
            )

        return candidates

    def perform_interleaved_actions(
        self,
        coverage_index,
        next_coverage_point,
    ):
        """在保持下一覆盖点不变的前提下插入少量低绕行动作。"""
        if not self.cfg.enable_interleaving:
            return
        ...
        while (
            completed
            < self.cfg.max_interleaved_actions_per_leg
        ):
            candidates = self.interleaved_candidates(
                next_coverage_point,
                measured_channels,
            )
            if not candidates:
            ...
            selected = max(
                candidates,
                key=lambda item: (
                    item["action_type"] == "CLEAR",
                    item["utility"],
                    -item["incremental_cost_s"],
                    -item["channel"],
                ),
            )
            self.event(
                "interleaved_action_selected",
                coverage_index=coverage_index,
                next_coverage_point=next_coverage_point,
                channel=selected["channel"],
                action_type=selected["action_type"],
                point=selected["point"],
                incremental_cost_s=selected[
                    "incremental_cost_s"
                ],
                route_detour_s=selected.get(
                    "route_detour_s",
                ),
                threshold_s=selected["threshold_s"],
                threshold_metric=selected[
                    "threshold_metric"
                ],
                diameter_reduction_m=selected[
                    "diameter_reduction_m"
                ],
                candidate_count=len(candidates),
            )

            self.counts["interleaved_action"] += 1
            record = selected["record"]
            before_distance = self.total_distance_m
            before_virtual = self.client.virtual_time_s

            if selected["action_type"] == "CLEAR":
                self.counts["interleaved_clear"] += 1
                self.clear(record, insertion=True)
            else:
                measured_channels.add(selected["channel"])
                self.measure(
                    record,
                    selected["point"],
                    measurement_kind="interleaved",
                )

            added_distance = self.total_distance_m - before_distance
            added_virtual = (
                self.client.virtual_time_s - before_virtual
            )
            self.interleaving_stats["distance_m"] += added_distance
            self.interleaving_stats[
                "virtual_time_s"
            ] += added_virtual
            self.event(
                "interleaved_action_completed",
                coverage_index=coverage_index,
                channel=selected["channel"],
                action_type=selected["action_type"],
                resulting_status=record.status,
                actual_distance_m=added_distance,
                actual_virtual_time_s=added_virtual,
            )
            completed += 1

    def perform_legacy_clear_insertion(
        self,
        next_coverage_point,
    ):
        """关闭V2.3交织时，保留V2.2的单次READY清除插入作为对照。"""
        if not self.cfg.enable_insert:
            return
    ...
    def search(self):
        ...
        for index, point in enumerate(self.points):
            ...
            # 3. 在当前覆盖边上交织低绕行的主动补测/可靠清除。
            if index < len(self.points) - 1:
                next_point = self.points[index + 1]
                if self.cfg.enable_interleaving:
                    self.perform_interleaved_actions(
                        index,
                        next_point,
                    )
                else:
                    self.perform_legacy_clear_insertion(
                        next_point,
                    )
    ...
    def summary(self, outcome, reason, exit_confirmed):
        ...
        return {"outcome":outcome, "reason":reason, "all_targets_resolved":self.complete(),
            ...
            "phase_virtual_s":self.phase_virtual_s,
            "interleaving_stats":self.interleaving_stats,
            "next_coverage_index":self.next_coverage_index,
...
def main():
    ...
    parser.add_argument("--interleave-detour", type=float, default=80.)
    parser.add_argument("--interleave-actions", type=int, default=2)
    ...
    config = Q3Config(rho=args.rho, enable_insert=not args.no_insert,
                      enable_interleaving=not args.no_interleave,
                      insert_threshold_s=args.insert_threshold,
                      interleave_detour_threshold_s=args.interleave_detour,
                      max_interleaved_actions_per_leg=args.interleave_actions,
    ...
    print(f"问题三 V2.3 启动，输出目录：{out}", flush=True)
    summary = runner.run()
