"""问题三基线+P2+动态重规划预算+P5+no_signal规划闭环。

同目录依赖（原文件内容无需修改，仅统一文件名）：
    client.py, geometry_V7.py, geometry_two_point_fast.py, planner.py
运行（模拟器已登录、已启动问题三测试且接口就绪）：
    python strategy_V2-2-2_no_signal.py --robot-id 你的参赛队号
关闭插入对照：
    python strategy_V2-2-2_no_signal.py --robot-id 你的参赛队号 --no-insert
参数对照：
    python strategy_V2-2-2_no_signal.py --robot-id 你的参赛队号 --rho 1123 --insert-threshold 60
清除失败上限：
    python strategy_V2-2-2_no_signal.py --robot-id 你的参赛队号 --max-clear-failures 3
...
class Q3Config:
    ...
    no_signal_repeat_m: float = 225.0
    ...
    max_replans: int = 3
    max_dynamic_replans: int = 6
    max_candidate_expansion_level: int = 3
    dynamic_replan_soft_budget_s: float = 300.0
    dynamic_replan_near_clear_radius_m: float = 80.0
    dynamic_replan_min_progress_rel: float = 0.02
    dynamic_replan_global_reserve_s: float = 600.0
    ...
    def __post_init__(self):
        ...
        positive_number_fields = (
            ...
            "dynamic_replan_near_clear_radius_m",
        )
        ...
        nonnegative_number_fields = (
            ...
            "progress_abs_m",
            "progress_rel",
            "dynamic_replan_min_progress_rel",
            "dynamic_replan_global_reserve_s",
        )
        ...
        if self.max_dynamic_replans < self.max_replans:
            raise ValueError(
                "max_dynamic_replans 不能小于max_replans"
            )

        if (
            type(self.max_candidate_expansion_level) is not int
            or self.max_candidate_expansion_level < 0
        ):
            raise ValueError(
                "max_candidate_expansion_level 必须是非负整数"
            )

        if self.dynamic_replan_min_progress_rel > 1.0:
            raise ValueError(
                "dynamic_replan_min_progress_rel 不应大于1"
            )
...
def reception_radius_interval(source, record, planner_cfg):
    """返回给定源假设仍可能成立的接收半径区间。

    direction 表明测量点位于真实接收半径内；no_signal 表明测量点位于
    真实接收半径外。该区间只服务于规划，绝不加入最终清除半平面。
    """
    ...
    upper = planner_cfg.reception_radius_max_m
    if record.no_signal_observations:
        upper = min(
            upper,
            *(
                math.dist(source, observation["position"])
...
def reception_history_feasible(source, record, planner_cfg):
    lower, upper = reception_radius_interval(
        source,
        record,
        planner_cfg,
    )
    return lower <= upper + 1e-7


def sampled_reception_fractions(sources, record, point, planner_cfg):
    guaranteed = 0
    possible = 0
    for source in sources:
        ...
        distance = math.dist(source, point)
        guaranteed += distance <= lower + 1e-7
        possible += distance <= upper + 1e-7
    count = len(sources)
    return guaranteed / count, possible / count


def sample_sources(record, cfg, planner_cfg):
    """从外包多边形及Q2的SourceRegion生成样本，并检查全部物理约束。

    no_signal 不裁剪最终清除几何，但会通过接收半径可行区间过滤规划样本。
    """
    rows = halfplanes(record)
    expansion_level = min(
        record.replan_level,
        cfg.max_candidate_expansion_level,
    )
    target = cfg.source_samples * (2 ** expansion_level)
    samples = []
    ...
    def add(p):
        ...
        if any(not 5.0 < math.dist(p, o["position"]) <= 1500+1e-7
            ...
            return
        if not reception_history_feasible(
            p,
            record,
            planner_cfg,
        ):
    ...
    # 当可行集合被 no_signal 圆盘切成窄条时，普通多边形面积采样可能漏掉它。
    # 围绕每个成功接收点沿方位角域直接补充射线样本。
    for observation_index, observation in enumerate(
        record.bearing_observations
    ):
        for index in range(1, target * 24 + 1):
            if len(samples) >= target:
                break
            radius = (
                planner_cfg.no_bearing_radius_m
                + 1e-4
                + (
                    planner_cfg.reception_radius_max_m
                    - planner_cfg.no_bearing_radius_m
                    - 1e-4
                )
                * radical_inverse(
                    index + observation_index * target * 24,
                    2,
                )
            )
            angle_offset = planner_cfg.bearing_error_deg * (
                2.0
                * radical_inverse(
                    index + observation_index * target * 24,
                    3,
            ...
            angle = math.radians(
                observation["bearing_deg"] + angle_offset
            )
            add(
                (
...
def candidate_min_active_no_signal_distance(record, candidate):
    """返回候选点到主动/恢复阶段无信号位置的最近距离。"""
    observations = [
        observation
        for observation in record.no_signal_observations
        if observation.get("measurement_kind") in {
            "active",
            "recovery",
        }
    ]
    if not observations:
        return math.inf
    return min(
        math.dist(candidate, observation["position"])
...
def lightweight_future_diameter(
    ...
    """兼容原调用，仅返回预测直径。"""
    geometry = lightweight_future_geometry(
        current_vertices,
        point,
        reported_bearing_deg,
        bearing_error_deg,
...
def score_candidate_geometry(
    ...
    current_radius = (
        ...
        else record.diameter / 2.0
    )
    worst_diameter = 0.0
    worst_radius = 0.0
    fallback_counts = {}
    guaranteed_receive_count = 0
    possible_receive_count = 0

    for source in sources:
        distance_to_source = math.dist(point, source)
        (
            reception_lower_bound,
            reception_upper_bound,
        ) = reception_radius_interval(
            source,
            record,
            planner_cfg,
        )
        ...
        if distance_to_source <= reception_upper_bound + 1e-7:
            possible_receive_count += 1

        # 只要某个仍可行的接收半径会返回 no_signal，单步几何最坏值就不缩小。
        if distance_to_source > reception_lower_bound + 1e-7:
            worst_diameter = max(
                worst_diameter,
                record.diameter,
        ...
        for error in errors:
            try:
                predicted_geometry = lightweight_future_geometry(
                ...
                fallback_counts["LIGHTWEIGHT_MEC_FAILURE"] = (
                    fallback_counts.get(
                        "LIGHTWEIGHT_MEC_FAILURE",
                        0,
                    )
                    + 1
                )
            ...
            if predicted_geometry is None:
                ...
                predicted_radius = current_radius
            else:
                predicted_diameter, predicted_radius = (
                    predicted_geometry
                )
            ...
            if predicted_radius > current_radius + 1e-5:
                fallback_counts["LIGHTWEIGHT_RADIUS_INCREASE"] = (
                    fallback_counts.get(
                        "LIGHTWEIGHT_RADIUS_INCREASE",
                        0,
                    )
                    + 1
                )
    ...
    reception_stats = {
        "guaranteed_receive_count": guaranteed_receive_count,
        "possible_receive_count": possible_receive_count,
        "guaranteed_receive_fraction": (
            guaranteed_receive_count / source_count
        ),
        "possible_receive_fraction": (
            possible_receive_count / source_count
        ),
    }
    return (
        worst_diameter,
...
def build_static_plan(
    ...
    # no_signal 出现后，不再只围绕可能严重偏移的多边形中心布点。
    # 从历史成功接收点朝源方向斜向推进，兼顾接收概率和交会角。
    if record.no_signal_observations:
        for observation in record.bearing_observations:
            for step in (200.0, 400.0, 600.0):
                for offset_deg in (-60.0, -45.0, -30.0,
                                   30.0, 45.0, 60.0):
                    angle = math.radians(
                        observation["bearing_deg"] + offset_deg
                    )
                    add_candidate(
                        (
                            observation["position"][0]
    ...
    count = cfg.candidate_count * (2 ** expansion_level)
    ...
    for candidate in candidates:
        ...
        min_active_no_signal_distance = (
            candidate_min_active_no_signal_distance(
                record,
                point,
            )
        )
        if min_active_no_signal_distance < cfg.no_signal_repeat_m:
            rejection_counts["near_active_no_signal"] += 1
            continue
        ...
        legal_candidates.append(
            {
                **candidate,
                ...
                "min_active_no_signal_distance_m": (
                    None
                    if math.isinf(min_active_no_signal_distance)
                    else min_active_no_signal_distance
                ),
            }
        )
        ...
        scored_candidates.append(
            {
                **candidate,
                ...
                "predicted_clearable": (
                    predicted_radius
                    <= 20.0 - cfg.clear_margin_m + 1e-9
                ),
                "predicted_radius_reduction_m": max(
                    0.0,
    ...
    return {
        ...
        "candidate_expansion_level": expansion_level,
...
def select_dynamic_proposal(
    ...
    best_radius = min(
        candidate["predicted_radius"]
        for candidate in available
    )
    clearable = [
        candidate
        for candidate in available
        if candidate["predicted_clearable"]
    ]
    radius_near_limit = (
        best_radius * (1.0 + cfg.near_optimal_rel)
        + cfg.near_optimal_abs_m
    )
    if clearable:
        near_optimal = clearable
        selection_stage = "direct_clear_target"
    else:
        radius_near_optimal = [
            candidate
            for candidate in available
            if candidate["predicted_radius"]
            <= radius_near_limit + 1e-9
        ]
        best_guaranteed_receive = max(
            candidate["guaranteed_receive_fraction"]
            for candidate in radius_near_optimal
        )
        if best_guaranteed_receive > 0.0:
            near_optimal = [
                candidate
                for candidate in radius_near_optimal
                if candidate["guaranteed_receive_fraction"]
                >= best_guaranteed_receive - 1e-12
            ]
            selection_stage = (
                "minimum_radius_then_guaranteed_receive"
            )
        else:
            best_possible_receive = max(
            ...
            near_optimal = [
                candidate
                for candidate in radius_near_optimal
                if candidate["possible_receive_fraction"]
                >= best_possible_receive - 1e-12
            ]
            selection_stage = (
                "minimum_radius_then_possible_receive"
            )
    ...
    return {
        ...
        "predicted_radius_reduction_m": selected[
            "predicted_radius_reduction_m"
        ],
        "selection_stage": selection_stage,
        "method": "q3_p5_no_signal_aware_then_route",
        ...
        "best_sampled_radius": best_radius,
        "direct_clear_candidate_count": len(clearable),
        "selected_guaranteed_receive_fraction": selected[
            "guaranteed_receive_fraction"
        ],
        "selected_possible_receive_fraction": selected[
            "possible_receive_fraction"
        ],
        "best_guaranteed_receive_fraction": max(
            candidate["guaranteed_receive_fraction"]
            for candidate in available
        ),
        "best_possible_receive_fraction": max(
            candidate["possible_receive_fraction"]
            for candidate in available
        ),
        "near_optimal_limit": radius_near_limit,
        "near_optimal_metric": "predicted_worst_mec_radius",
        ...
        "min_active_no_signal_distance_m": selected[
            "min_active_no_signal_distance_m"
        ],
        "candidate_rejections": rejection_counts,
...
class Q3Runner:
    ...
    def __init__(self, client, *, config=None, planner_config=None, geometry_config=None,
        ...
        self.counts = {
            ...
            "dynamic_replan_extra_granted": 0,
            "dynamic_replan_budget_exhausted": 0,
        }
    ...
    def predict_opportunistic_radius(
        self,
        record,
        point,
        sources=None,
        ...
        for channel, record in self.records.items():
            ...
            try:
                ...
                self.event(
                    ...
                    arrival_kind=arrival_kind,
                    primary_channel=primary_channel,
                    reason="no_reception_feasible_source_sample",
                    detail=str(error),
                )
                continue
            ...
            (
                guaranteed_receive_fraction,
                possible_receive_fraction,
            ) = sampled_reception_fractions(
                sources,
                record,
                point,
                self.pcfg,
            )
            if possible_receive_fraction <= 0.0:
                continue
            # 已经发生过 no_signal 时，低于一半样本仍可能接收的顺便测量
            # 通常只会继续消耗5~6秒；主动规划会负责寻找接收恢复点。
            if (
                record.no_signal_observations
                and possible_receive_fraction < 0.5
            ):
                continue
            ...
            reduction_rel = (
                reduction / record.clearance_radius
                if record.clearance_radius > 1e-9
                else 0.0
            )
            predicted_clearable = (
                predicted_radius
                <= 20.0 - self.cfg.clear_margin_m + 1e-9
            )
            operation_s = (
                5.0
            ...
            candidates.append(
                {
                    ...
                    "benefit": benefit,
                    "predicted_radius": predicted_radius,
                    "predicted_clearable": predicted_clearable,
                    "radius_reduction": reduction,
                    "radius_reduction_rel": reduction_rel,
                    "cross_angle_deg": cross_angle,
                    "min_history_distance_m": min_distance,
        ...
        while (
            ...
            selected = max(
                ...
                key=lambda item: (
                    ...
                    item["guaranteed_receive_fraction"],
            ...
            self.event(
                ...
                radius_reduction_rel=selected[
                    "radius_reduction_rel"
                ],
                guaranteed_receive_fraction=selected[
                    "guaranteed_receive_fraction"
                ],
                possible_receive_fraction=selected[
                    "possible_receive_fraction"
                ],
                cross_angle_deg=selected["cross_angle_deg"],
                min_history_distance_m=selected[
    ...
    def planning_cache_key(self, record):
        ...
        return (
            ...
            self.cfg.clear_margin_m,
            self.cfg.max_candidate_expansion_level,
            self.cfg.repeated_point_m,
            self.cfg.no_signal_repeat_m,
            self.cfg.min_cross_angle_deg,
            self.cfg.near_optimal_abs_m,
            self.cfg.near_optimal_rel,
        )

    def dynamic_replan_budget(self, record):
        """基础次数不减少；仅把额外次数分配给近清除或仍有进展的频道。"""
        recent_progress_rel = 0.0
        for item in record.progress_history[-3:]:
            before = item.get("radius_before")
            after = item.get("clearance_radius")
            if (
                before is not None
                and after is not None
                and before > 1e-9
            ):
                recent_progress_rel = max(
                    recent_progress_rel,
                    max(0.0, before - after) / before,
                )

        current_radius = record.clearance_radius
        near_clear = (
            current_radius is not None
            and current_radius
            <= self.cfg.dynamic_replan_near_clear_radius_m
        )
        pending_count = max(
            1,
            sum(
                item.status in {"FOUND", "READY"}
        ...
        if (
            ...
            global_remaining_s = math.inf
        else:
            global_remaining_s = max(
                0.0,
                self.client.max_virtual_duration_s
                - self.client.virtual_time_s
                - self.cfg.dynamic_replan_global_reserve_s,
            )
        fair_share_s = global_remaining_s / pending_count

        extra = 0
        reasons = []
        if fair_share_s >= self.cfg.dynamic_replan_soft_budget_s:
            if near_clear:
                extra += 2
                reasons.append("near_clear_target")
            if (
                recent_progress_rel
                >= self.cfg.dynamic_replan_min_progress_rel
            ):
                extra += 1
                reasons.append("recent_effective_progress")
            if record.clear_failure_count > 0:
                extra += 1
                reasons.append("clear_failure_recovery")

        limit = min(
            self.cfg.max_dynamic_replans,
            self.cfg.max_replans + extra,
        )
        return {
            "limit": limit,
            "base_limit": self.cfg.max_replans,
            "extra": limit - self.cfg.max_replans,
            "near_clear": near_clear,
            "current_radius": current_radius,
            "recent_progress_rel": recent_progress_rel,
            "pending_count": pending_count,
            "global_remaining_s": global_remaining_s,
            "fair_share_s": fair_share_s,
            "reasons": reasons,
        }
    ...
    def replan(self, record, reason):
        budget = self.dynamic_replan_budget(record)
        record.failure_count += 1
        record.replan_level += 1
        record.no_progress_count = 0
        ...
        record.q2_attempted = True
        if (
            record.replan_level > self.cfg.max_replans
            and record.replan_level <= budget["limit"]
        ):
            self.counts["dynamic_replan_extra_granted"] += 1
        if record.replan_level > budget["limit"]:
            self.counts["dynamic_replan_budget_exhausted"] += 1
        self.event(
            "replan",
            channel=record.channel_id,
            level=record.replan_level,
            reason=reason,
            dynamic_budget=budget,
            accepted=record.replan_level <= budget["limit"],
        )
    ...
    def localize(self):
        ...
        while any(
            ...
            for channel, record in self.records.items():
                ...
                replan_budget = self.dynamic_replan_budget(record)
                if (
                    record.status == "FOUND"
                    and record.replan_level
                    > replan_budget["limit"]
                ):
                    continue
...
def main():
    ...
    parser.add_argument("--rho", type=float, default=1130.)
    ...
    parser.add_argument("--max-dynamic-replans", type=int, default=6)
    parser.add_argument("--replan-soft-budget", type=float, default=300.)
    parser.add_argument("--no-signal-repeat", type=float, default=225.)
    ...
    config = Q3Config(rho=args.rho, enable_insert=not args.no_insert,
                      ...
                      max_dynamic_replans=args.max_dynamic_replans,
                      dynamic_replan_soft_budget_s=args.replan_soft_budget,
                      no_signal_repeat_m=args.no_signal_repeat)
    ...
    print(
        "问题三基线+P2+动态预算+P5+no_signal规划闭环启动，"
        f"输出目录：{out}",
        flush=True,
    )
    summary = runner.run()
