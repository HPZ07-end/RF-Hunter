"""问题三 V2.2 主程序：七点覆盖、失败重定位、安全退出与动态开放TSP。
...
class Q3Config:
    ...
    # 20°为硬约束，30°以上作为优选条件。
    min_cross_angle_deg: float = 20.0
    preferred_cross_angle_deg: float = 30.0

    # 新测量至少压缩20%的当前几何不确定性。
    min_candidate_reduction_rel: float = 0.20
    ...
    def __post_init__(self):
        ...
        if self.candidate_count > 8 or self.candidate_count % 2:
            raise ValueError(
                "candidate_count 必须是一个不大于8的正偶数"
            )

        required_offsets = self.candidate_count // 2
        if len(self.pca_candidate_offsets_m) < required_offsets:
            raise ValueError(
                "pca_candidate_offsets_m 数量不足"
            )

        if any(
            not math.isfinite(offset) or offset <= 0
            for offset in self.pca_candidate_offsets_m
        ):
            raise ValueError(
                "PCA候选距离必须全部为有限正数"
            )
        ...
        if not (
            self.min_cross_angle_deg
            <= self.preferred_cross_angle_deg
            <= 90.0
        ):
            raise ValueError(
                "preferred_cross_angle_deg 必须位于"
                "[min_cross_angle_deg,90] 内"
            )

        if not 0.0 <= self.min_candidate_reduction_rel < 1.0:
            raise ValueError(
                "min_candidate_reduction_rel 必须位于 [0,1) 内"
            )
        ...
        if self.near_optimal_rel > 1.0:
            raise ValueError("near_optimal_rel 不应大于1")
...
def all_measurement_observations(record):
    """返回该频道全部已经实际测试过的位置记录。"""
    return (
...
def candidate_min_history_distance(record, candidate):
    """候选点与全部历史测试位置之间的最小距离。"""
    observations = all_measurement_observations(record)
...
def point_already_tested(record, point, tolerance=1e-6):
    return any(
        math.dist(point, observation["position"]) <= tolerance
        for observation in all_measurement_observations(record)
    )
...
def feasible_region_principal_axes(record, sources):
    """计算当前可行域中心、长轴、短轴及两个协方差特征值。"""
    points = deduplicate_vertices(
        list(record.outer_polygon) + list(sources)
    )

    if not points:
        raise IncompleteRun(
            f"频道{record.channel_id}没有可用于PCA的可行域样本"
        )
    ...
    if cov_xx + cov_yy <= 1e-12:
        long_axis = (1.0, 0.0)
        short_axis = (0.0, 1.0)
        return center, long_axis, short_axis, (0.0, 0.0)
...
def build_static_plan(
    ...
    (
        ...
        short_axis,
        eigenvalues,
    ) = feasible_region_principal_axes(record, sources)
    ...
    offset_count = cfg.candidate_count // 2

    # 重规划时仍保持8点结构，只适度扩大测量基线。
    replan_scale = 1.0 + 0.25 * min(
        record.replan_level,
        cfg.max_replans,
    )

    offsets = cfg.pca_candidate_offsets_m[:offset_count]

    for offset in offsets:
        effective_offset = offset * replan_scale

        for sign in (-1.0, 1.0):
            add_candidate(
                (
                    estimate[0]
                    + sign * effective_offset * short_axis[0],
                    estimate[1]
                    + sign * effective_offset * short_axis[1],
                ),
                "pca_short_axis",
            )
    ...
    if not candidates:
        raise IncompleteRun(
            f"频道{record.channel_id}的PCA候选点全部位于目标区域外"
        )
        ...
        legal_candidates.append(
            {
                **candidate,
                ...
                "preferred_cross_angle": (
                    cross_angle
                    >= cfg.preferred_cross_angle_deg
                ),
                "min_history_distance_m": (
                    min_history_distance
                ),
            }
        )
    ...
    if not legal_candidates:
        raise IncompleteRun(
            f"频道{record.channel_id}的PCA候选均不满足"
            f"{cfg.repeated_point_m}米距离或"
            f"{cfg.min_cross_angle_deg}度交会角约束"
        )

    scored_candidates = []
    prediction_fallbacks = {}

    if record.diameter is None or record.diameter <= 1e-9:
        raise IncompleteRun(
            f"频道{record.channel_id}当前直径无效，不能评价候选"
        )
    ...
    for candidate in legal_candidates:
        ...
        for status, count in fallbacks.items():
            prediction_fallbacks[status] = (
                prediction_fallbacks.get(status, 0)
                + count
            )

        reduction = max(
            0.0,
            record.diameter - predicted_diameter,
        )
        reduction_rel = reduction / record.diameter

        if (
            reduction_rel + 1e-12
            < cfg.min_candidate_reduction_rel
        ):
            rejection_counts["insufficient_reduction"] += 1
            continue
        ...
        scored_candidates.append(
            {
                **candidate,
                ...
                "predicted_reduction_m": reduction,
                "predicted_reduction_rel": reduction_rel,
            }
        )

    if not scored_candidates:
        raise IncompleteRun(
            f"频道{record.channel_id}没有预测压缩率达到"
            f"{100.0 * cfg.min_candidate_reduction_rel:.1f}%"
            "的PCA候选点"
        )

    return {
        "method": "q3_pca_short_axis_geometry",
        "estimate": estimate,
        "long_axis": long_axis,
        ...
        "score_scope": (
            "pca_short_axis_all_bearings_and_prior_box"
        ),
    }
...
def select_dynamic_proposal(
    ...
    selected = min(
        ...
        key=lambda candidate: (
            not candidate["preferred_cross_angle"],
    ...
    return {
        ...
        "preferred_cross_angle": selected[
            "preferred_cross_angle"
        ],
        "predicted_reduction_m": selected[
            "predicted_reduction_m"
        ],
        "predicted_reduction_rel": selected[
            "predicted_reduction_rel"
        ],
    }
...
class Q3Runner:
    ...
    def opportunistic_candidates(self, coverage_index, point):
        ...
        for channel, record in self.records.items():
            ...
            if record.clearance_radius is None or record.clearance_radius <= 1e-9:
                continue
            ...
            reduction_rel = reduction / record.clearance_radius

            if (
                reduction_rel + 1e-12
                < self.cfg.min_candidate_reduction_rel
            ):
                continue
    ...
    def planning_cache_key(self, record):
        ...
        return (
            ...
            self.cfg.preferred_cross_angle_deg,
            self.cfg.min_candidate_reduction_rel,
...
def main():
    ...
    print(f"问题三 V2.2 启动，输出目录：{out}", flush=True)
    summary = runner.run()
